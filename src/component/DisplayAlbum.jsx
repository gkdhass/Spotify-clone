    ...
    <>
      ...
      {songsData.map((item, index) => (
        <div
          ...
          key={index}
          className="grid grid-cols-3 sm:grid-cols-4 p-2 items-center text-[#a7a7a7] hover:bg-[#ffffff2b] cursor-pointer"
        >
          <p className="text-white">
            <b className="mr-4 text-[#a7a7a7]">{index + 1}</b>
            <img className="inline-block w-10 mr-5" src={item.image} alt={item.name} />
            {item.name}
          </p>
